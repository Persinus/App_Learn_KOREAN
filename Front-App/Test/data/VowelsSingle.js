...
export default function VowelsSingleSingle() {
  ...
  const [selectedCardIndex, setSelectedCardIndex] = useState(null); // Track the selected card
  ...
  const speakLetter = (transliteration, index) => {
    ...
    setIsSpeaking(true);
    setSelectedCardIndex(index); // Highlight the selected card

    Speech.speak(`${transliteration}ㅏ`, {
      language: 'ko', // Korean
      pitch: 1.5, // Voice pitch
      rate: 0.99, // Speaking rate
      ...
      <View style={styles.cardContainer}>
        {vowelsSingle.map((item, index) => {
          ...
          return (
            <TouchableOpacity
              key={index}
              onPress={() => speakLetter(item.letter, index)}
              style={[
                styles.card,
                {
                ...
                isSelected && styles.cardSelected, // Highlight the selected card
              ]}
            >
...
const styles = StyleSheet.create({
  ...
  cardSelected: {
    ...
    shadowRadius: 15, // Larger shadow for glow effect
    transform: [{ scale: 1.1 }], // Scale up the selected card
    borderWidth: 3,
    borderColor: '#FFFFFF', // Add white border for extra emphasis
  },
  cardContent: {
    alignItems: 'center',
  },
  ...
  letterSelected: {
    ...
    textShadowColor: '#000000', // Add shadow to make letter pop
