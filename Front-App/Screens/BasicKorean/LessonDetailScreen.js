...
const LessonDetailScreen = ({ route, navigation }) => {
  ...
  const renderIntroduction = () => (
    <View style={styles.sectionContainer}>
      <Text style={styles.sectionText}>{lessonContent.introduction.content}</Text>
      
      <Text style={styles.goalTitle}>Mục tiêu bài học:</Text>
    ...
    <View style={styles.sectionContainer}>
      {lessonContent.vocabulary.map((item, index) => (
        <View key={index} style={styles.vocabularyItem}>
          ...
          >
            <Text style={styles.koreanWord}>{item.word}</Text>
            <FontAwesome5 
          ...
          <View style={styles.exampleContainer}>
            <Text style={styles.exampleKorean}>{item.example}</Text>
          ...
          <View style={styles.examplesContainer}>
            {point.examples.map((example, i) => (
              <View key={i} style={styles.grammarExample}>
                <Text style={styles.grammarKorean}>{example.korean}</Text>
                <Text style={styles.grammarTranslation}>{example.translation}</Text>
              </View>
            ))}
          </View>
        </View>
  ...
  const renderConversation = () => (
    <View style={styles.sectionContainer}>
        ...
        <View key={index} style={styles.dialogue}>
          ...
          {dialogue.speakers.map((speaker, i) => (
            <View key={i} style={styles.dialogueLine}>
              ...
              >
                <Text style={styles.dialogueText}>{speaker.text}</Text>
                <FontAwesome5 name="volume" size={12} color="#4b46f1" style={styles.volumeIcon} />
              </TouchableOpacity>
              <Text style={styles.dialogueTranslation}>{speaker.translation}</Text>
            </View>
          ))}
        </View>
      ))}
      ...
      <TouchableOpacity 
        style={styles.conversationPracticeButton}
        onPress={() => navigation.navigate('ConversationPracticeScreen', { lessonId: lesson.id })}
      >
        <Text style={styles.conversationPracticeText}>Luyện tập hội thoại</Text>
  ...
  const renderQuiz = () => (
    <View style={styles.sectionContainer}>
      <Text style={styles.quizDescription}>{lessonContent.quiz.description}</Text>
      
      <TouchableOpacity 
        style={styles.quizButton}
        onPress={() => navigation.navigate(lessonContent.quiz.screen, { lessonId: lesson.id })}
      >
        <Text style={styles.quizButtonText}>Bắt đầu kiểm tra</Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={headerStyles.container}>
        ...
        <Text style={headerStyles.title}>{lesson.title}</Text>
      </View>
        ...
        >
          {sections.map((section, index) => (
            <TouchableOpacity
              ...
              style={[
                styles.tabButton,
                activeSection === index && styles.activeTabButton
              ]}
...
const styles = StyleSheet.create({
  ...
  // Styles for conversation section
  dialogue: {
    marginBottom: 20,
  },
  dialogueTitle: {
